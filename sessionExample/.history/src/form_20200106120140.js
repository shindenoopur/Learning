...
class Form extends Component{
    ...
    handleupdate =  (updatedState) => {
        const oldUsername = this.props.characters[0].username
        console.log("oldUsername", oldUsername)
        console.log("this.state in App", this.state, updatedState.username, "chanracters", this.props.characters)
        const index = this.props.characters.findIndex(p => p.username === oldUsername)
        console.log("Index at",index)
        if(index!== -1){
        
        this.setState(prevState => {
            if(updatedState.username!=='') 
            this.props.characters[index].username = updatedState.username;
            if(updatedState.email !== '')
            this.props.characters[index].email = updatedState.email;
        });
    ...
    if(this.state.passwordState === "weak"){
      ...
      passwordStrong=true;
       ...
       console.log("this.props.editing", this.props.editing, "thhis.editing",this.state.editing)
    
      this.state.updatedUser = this.props.changedValue
        if(this.props.editing === true && this.editing===undefined){
            console.log("Ifdddf", this.props.editing, "ddd", this.editing)
            // console.log("this.state.username", this.state.username)
