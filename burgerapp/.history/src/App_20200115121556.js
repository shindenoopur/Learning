...
class App extends Component {
  ...
  onchangeHandler = (event, id) => {
    ...
    const person = {...this.state.persons[toChangeIndex]}
    ...
    if(this.state.showPersons){
       ...
       <div className="Persons">
          {this.state.persons.map((person,index) => {
            return <Person name={person.name} age={person.age} key={person.id} onChange ={(event) => {this.onchangeHandler(event, person.id)}} ondeleteHandle = {() => {this.deleteHandler(index)}}/>
          })}
