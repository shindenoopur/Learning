...
class Form extends Component{
  ...
  handleupdate =  (username) => {
    ...
    submitForm = (event) => {
      ...
      updateForm = (event) => {
        ...
        // if(!validate){
            ...
            // this.setState(this.initialState),
            this.handleupdate(this.state.username)
        // }
      }
    render(){
    ...
    if(this.state.passwordState === "weak"){
      ...
      console.log("changedValue:", this.state.updatedUser);
      const updatedUser = this.state.updatedUser;
        if(this.props.editing === true){
            console.log("this.state.username", this.state.updatedUser)
            return( <div className="col-md-12">
                        <form>
                            <label>UserName</label>
                            <input type="text" name="username" value={this.state.updatedUser} onChange={this.updateDetails}/>
